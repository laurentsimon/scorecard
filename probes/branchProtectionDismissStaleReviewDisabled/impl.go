...
package branchProtectionDismissStaleReviewDisabled
...
var probe = "branchProtectionDismissStaleReviewDisabled"
...
func handleSetting(branch clients.BranchRef) (*finding.Finding, error) {
	...
	if *branch.BranchProtectionRule.RequiredPullRequestReviews.DismissStaleReviews {
		f, err := finding.NewNegative(fs, probe,
			fmt.Sprintf("setting enabled for '%s'", *branch.Name), nil)
		if err != nil {
			return nil, fmt.Errorf("create finding: %w", err)
		}
		return f, nil
